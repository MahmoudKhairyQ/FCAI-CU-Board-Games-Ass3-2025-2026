...
public:
    ...
    int getBestMove(const vector<vector<char>>& board) {
        ...
        // Get all available columns (check from top row)
        ...
        if (available.empty()) {
            return 3; // Default to center if no available columns
        }
        ...
        // 3. Prefer center columns for better positioning
...
Move<char>* FourInARowUI::get_move(Player<char>* player) {
    ...
    // Board will determine the correct row (lowest empty spot)
