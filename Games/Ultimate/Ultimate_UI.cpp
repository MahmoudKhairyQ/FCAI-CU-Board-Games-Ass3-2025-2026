...
UltimateUI::UltimateUI(string message) : UI<char>(message, 3) {
...
bool UltimateUI::is_playable_cell(const vector<vector<char>>& mat, int row, int col) const {
    // Check bounds
...
Move<char>* UltimateUI::get_move(Player<char>* player) {
    ...
    UltimateBoard* pb = nullptr;
    if (bptr) {
        // board pointer may point to a Pyramid instance
        pb = dynamic_cast<UltimateBoard*>(bptr);
    }
    ...
    while (true) {
        ...
        cout << "\n coordinates (row col). Playable cells are shown as '.'\n";
